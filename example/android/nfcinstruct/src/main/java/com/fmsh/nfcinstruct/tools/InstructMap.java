...
public class InstructMap {
    ...
    public static List<byte[]> getBaseInstructList(boolean nfcA) {
        ...
        if (nfcA) {
            byteList.add(instructA(34));//获取场强指令
            byteList.add(instructA(11));//启动实时测温
            byteList.add(instructA(12));// 获取实时测温数据
            byteList.add(instructA(9));//查看状态结果
            byteList.add(instructA(28));//获取电池电压第一步 配置寄存器
            byteList.add(instructA(29));//获取电池电压第二步  启动测量
            byteList.add(instructA(30)); //获取电池电压第三步 读取结果
            byteList.add(instructA(31));//获取电池电压第四步  恢复寄存器
        } else {
    ...
    public static List<byte[]> getParameterInstructList(boolean nfcA) {
        ...
        if (nfcA) {
            byteList.add(instructA(9));//查看状态结果
            byteList.add(instructA(1));//唤醒指令
            byteList.add(instructA(2));//查看唤醒状态
            byteList.add(instructA(3, parameterArr[0]));//配置延时测温时间 m
            byteList.add(instructA(5, parameterArr[1]));//配置测温间隔时间 s
            byteList.add(instructA(13, parameterArr[2]));//设置测温次数
            byteList.add(instructA(40));//写测温最小值
            byteList.add(instructA(41)); //写测温最大值
            byteList.add(instructA(16, parameterArr[3])); // 设置最小温度值
            byteList.add(instructA(17, parameterArr[4]));// 设置最大温度值
            byteList.add(instructA(19, parameterArr[0])); // 延时测温时间
            byteList.add(instructA(20, parameterArr[1]));// 写测温间隔
            byteList.add(instructA(22));//写时间
            byteList.add(instructA(7));//开启rtc测温
            byteList.add(instructA(44)); // 自己加的，获取记录次数
        } else {
            byteList.add(instructV(9));
    ...
    public static List<byte[]> getStopInstruct(boolean nfcA) {
        List<byte[]> byteList = new ArrayList<>();
        if (nfcA) {
            byteList.add(instructA(35));//获取随机数
            byteList.add(instructA(53));//获取auth_rb_cfg
            byteList.add(instructA(55));//密码认证
            byteList.add(instructA(45));//停止测温发送随机数指令
        } else {
    ...
    public static List<byte[]> getResultInstruct(boolean nfcA){
        List<byte[]> byteList = new ArrayList<>();
        if (nfcA) {
            byteList.add(instructA(46));// 读取测温数据指令
            byteList.add(instructA(14)); //查看测温次数
            byteList.add(instructA(21));//读取测温间隔 和延时测温时间
            byteList.add(instructA(23));//读测温时间
            byteList.add(instructA(18));//获取最大最小温度值
            byteList.add(instructA(36));//读取c094寄存器
            byteList.add(instructA(37));//读取B180地址
            byteList.add(instructA(38));//读取c09a寄存器max_limit_cnt
            byteList.add(instructA(39));//读取c09b寄存器min_limit_cnt
            byteList.add(instructA(42));//读测温最小值
            byteList.add(instructA(43));//读测温最大值
            byteList.add(instructA(4));//查看延时测温时间
            byteList.add(instructA(9));//查看状态结果
            byteList.add(instructA(32));//读C091寄存器
            byteList.add(instructA(44));//读测温次数
            byteList.add(instructA(47));// 读取温度记录数据
            byteList.add(instructA(48));//获取配置信息,检测当前标签是否为体温标签
        } else {
